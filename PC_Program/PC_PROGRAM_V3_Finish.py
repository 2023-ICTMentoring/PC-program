...
def XO_attend(): 
    ...
    #A1_output=tk.Entry(text = "인원수", font=('맑은 고딕',30),bg='white',width=8)
    #현재 출석 인원 받아서 넣어주기
    #A1_output.pack()
...
def Korean_go():   
    ...
    def two_word():
        ...
        while ucpass < how_many_student:
            if expected_addr > how_many_student:
                break
            random_font = random.randrange(0, 19)
            random_font_list.append(random_font)
            send_yes_data_to_student(py_serial, int(random_addr[ucpass], 16), CMD_CHOSUNG_FONT, random_font_list[ucpass])
            time.sleep(0.2)
            expected_addr += 1
            ucpass += 1
